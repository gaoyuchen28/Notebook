...
>  Transcriptomics: Gene Expression Profiling of Cells
> 
> - Bulk transcriptomics
> - Single Cell transcriptomics
> - Spatial transcriptomics

### BASIC CONCEPTS OF A CELL

All biological systems are composed of cells containing the **same types** of chemical molecules and employing **similar principles** of organization at the cellular level.
And that's why  investigations of **model organisms** informative for understanding fundamental cellular processes.

#### What is cell

Cells are the fundamental unit of life(**The cell theory**)
- The simplest organisms are single cells
- Cells are the building blocks of tissues
- All living cells are derived from pre-existing cells
...
Different types of organisms all initiate from a very similar form of life.

The hereditary information in the fertilized eggs determines the nature of the whole organism that will develop from it.

> **Model Organism**: The genome of sea urchins encodes for many **vertebrate immune system** related genes.

### THE UNIVERSAL FEATURES OF CELLS ON EARTH

Certain small molecules (monomers) can be joined to form polymers (also called macromolecules): polysaccharides, proteins, and nucleic acids
...
- it alse can have their special property eg,the length of the tail
All Cells Are Enclosed in a Plasma Membrane Across Which Nutrients and Waste Materials Must Pass

**3 common features of all cells: Cellular membrane, cytoplasm, genetic materials**<br>
**Diversity: size, shape, behavior**<br>
**Specialization: same DNA, but different RNA and express pattern**<br>

#### Prokaryotic Cell Structure and Function

- Gram Negative & Gram positive
  - Gram positive bacteria have a thick peptidoglycan layer and no outer lipid membrane 
  - Gram negative bacteria have a thin peptidoglycan, but it has outer membrane so that lysosome can't digest the peptidoglycan.
...
- **Nucleus**
  - outer membrane connact to ER membrane
...
- **Golgi Complex and Endoplasmic Reticulum(ER)**
  ...
  - Rough ER: site of protein synthesis (lysosomal, membrane, secreted proteins)
    - protein folding
    - N-glycosylation
  - Smooth ER(tubule-like): 
    - lipid synthesis (fatty acids and phospholipids)
    ...
    - Ca2+ storage: e.g. sarcoplasmic reticulum in the **muscle cells**
  - Golgi apparatus: each layer is different
    - cis face(close to ER,entry) and trans face(exit)
    - Receiving vesicles from rER and sER
    - Protein modifications:N- and O- glycosylation, phosphorylation
...
- temperature, pH, ionic strength, nutrients
- specially coated plastic dishes/flasks: importance of cell-adhesion molecules (CAM)
- cell culture hood (sterile cabinet containing circulating air that is filtered)
...
- Glutamine：Nutrient source
- Vitamins, various salts, fatty acids, glucose
- Serum: protein factors for cell proliferation (insulin, transferrin, growth factors)
- Antibiotics: reduce bacterial or fungal contamination

**Comparison: Primary Cells vs. Transformed Cells**

| Feature              | Primary Cells                                | Transformed Cells (Cell Lines)          |
| :------------------- | :------------------------------------------- | :-------------------------------------- |
| **Origin** | Isolated directly from living tissues        | Derived from tumors or oncogenic shift  |
| **Isolation** | Requires Trypsin/Collagenase + EDTA          | Established from existing cultures      |
| **Life Span** | **Finite** (e.g., ~50 divisions; Senescence) | **Indefinite** (Immortalized)           |
| **Genetics** | Normal diploid; Genetically stable           | **Aberrant** chromosomes; DNA mutations |
| **Growth Potential** | High (Embryonic Stem Cells as exception)     | Continuous proliferation (e.g., HeLa)   |
| **Differentiation** | Can differentiate into specific lineages     | Often lose specialized functions        |
...
- Upright microscope
- Inverted microscope: look at cells in culture

- The light microscope can resolve details 0.2 μm apart
  - Resolution分辨率 (the limiting separation at which two objects appear distinct): the most important property of any microscope
- Bacteria and mitochondria are about 500 nm (0.5 μm) wide

**Ways to generate contrast in light microscopy**

- The stained portion of the cell will absorb light of some wavelengths, which depends on the stain, but will allow other wavelengths to pass through it.（入射都是白光，但是染色的部分类似于滤光片）
...
- sectioned: Microtome(Typically 0.5-10 μm thick)
- Fixative: a chemical that cross-link most proteins and nucleic acids(eg:formaldehyde)
- Embedding media are usually waxes(蜡) or resins(树脂): In liquid form, these media both permeate and surround the fixed tissue before being hardened (by cooling or by polymerization) to form a solid block.
...
Illuminate a fluorescent molecule at its absorbing wavelength and then view it through a filter that allows only light of the emitted wavelength to pass.

Dichroic mirror: a special type to filter that reflects the excitation light into the sample and allows the longer wavelength light emitted by the fluorescent sample to pass through to the observer 简单来说就是能够分辨出哪个是emission的光，然后把所有用来激发的光都避开

> Two fluorescent dyes that have been commonly used:
> Fluorescein-- emits an intense **green** fluorescence when excited with **blue** light
> Rhodamine-- emits a deep **red** fluorescence when excited with **green yellow** light.
> 这两个可以一起标记然后分辨不同的细胞器

**In situ(原位杂交) hybridization can reveal the location of RNAs**

It is possible to visualize the distribution of **different RNAs** in tissues using in situ (in the original place) hybridization
Tissues are gently fixed an hybridized with a labeled complementary nucleic acid probe.

**Antibodies Can Be Used to Detect Specific Molecules**

Polyclonal Antibodies
- Immunizing an animal with the appropriate antigen.
- Antibodies can then be purified from the serum.
- this will be a mix of antibodies
...
- The fluorescent light emitted by a sample comes not only from the plane of focus but also from molecules above and below it.
- To visualize thick specimens, consecutive (serial) images at various depths throughout the sample must be collected and then aligned to reconstruct structures in the original thick tissue.
  
  solutions:
  - Deconvolution: uses computational methods to remove fluorescence contributed by out-of-focus parts of the sample
  - Confocal Microscopy
...
- Measuring the distance between entities, such as proteins.
- FRET (no signal)->Distance >10-100 angstroms
- FRET (fluorophores are close enough together such that YFP can absorb light from CFP emission) Distance < 10-100 angstroms

Fluorescence Recovery after Photobleaching (FRAP)
- Measuring lateral diffusion rates of integral membrane protein
- Measuring lipids movement on cell plasma membrane

**Flow cytometry(流式细胞术)**

FACS (Fluorescence-activated cell sorter)
- 液滴形成（Droplet Formation）：样本被稀释成单细胞悬液，通过一个振动的喷嘴。振动使液流断裂成一个个微小的液滴，理想情况下，每个液滴只含有一个细胞。
- 激光检测与分析：在液滴形成前，细胞经过激光照射。仪器通过传感器检测其散射光（代表大小和复杂程度）和荧光（代表标记的特定蛋白或核酸）。
- 液滴充电（Charging）：计算机根据预设的“逻辑门”（Gating）判断该细胞是否为目标细胞。如果是，仪器会在液滴脱离喷嘴的一瞬间给该液滴带上正电荷或负电荷；非目标细胞则不带电。
- 电偏转分选（Deflection）：液滴通过一对高压静电偏转板。带电液滴受电场力作用发生偏转，落入指定的收集管中，而不带电的液滴则直接落入废液桶。

#### The Electron Microscope

In EM, electromagnetic lenses focus a high-velocity electron beam instead of the visible light used by optical lenses.

> $$d \approx \frac{\lambda}{2 \cdot \text{NA}}$$

Short wavelength of electrons means that the limit of resolution for TEM is theoretically 0.005nm.

- Because electrons are scattered by collisions with air molecules, air must first be pumped out of the column to create a vacuum.
- As in light microscopy, the specimen can be stained—in this case, with electron-dense material(能够强烈散射电子).
- The image can be observed on a monitor or is typically recorded with a sensitive CMOS electron detector. Because the scattered electrons are lost from the beam, the dense regions of the specimen show up in the image as areas of reduced electron flux, which look dark.

**Biological Specimens Require Special Preparation for Electron Microscopy**
- glutaraldehye; osmium tetroxide
- dehydration(保证在真空中不爆裂); solid plastic block(先填充聚合，然后用金刚石切片机切出纳米级的薄片)
- 50 -100 nm thick since electrons have limited penetrating power

**Localizing Proteins for Electron Microscopy**

Immunogold(免疫金) EM
-  Incubate a thin section first with a specific primary antibody
-  Incubate with a secondary antibody to which a colloidal gold particle has been attached.
   -  The gold particle is electron-dense and can be seen as a black dot in the electron microscope.
...
- 磁线圈产生的磁场会对带电的电子束施加洛伦兹力，使其发生偏转并聚焦。
- 扫描线圈 (Scan Coils) 控制电子束在样品表面的 $X-Y$ 轴上快速往复运动。
- 电子束每到达一个位置（像素点），都会与样品发生相互作用
- While the TEM uses the electrons that have passed through the specimen to form animage, the SEM uses electrons that are scattered or emitted from the specimen’s surface.

#### ANALYZING AND MANIPULATING DNA

**Hybridization Provides a Powerful, But Simple Way to Detect Specific Nucleotide Sequences**

> Diagnosis of sickle-cell anemia by DNA hybridization

Fluorescent in situ hybridization (FISH) can be used to localize genes on chromosomes
...
> VNTRs do not code for proteins, mutations in a VNTR do not alter biological function
> - Tandem repeats are common in noncoding regions of DNA 
...
The lipid component consists of millions of lipid molecules arranged in two closely apposed sheets (lipid bilayer)
Eucaryotic organisms have internal membranes which enclose internal compartments
...
The lipid bilayer is about **5 nm** or **50 atoms** thick
...
- The most abundant type of PL is **phosphatidylcholine**
...
- sterol: stablize the membrane
  ...
  -  It fills in the gaps generated by kinks in unsaturated hydrocarbon tails
  -  Helps to stiffen membranes
...
> Langmuir-Blodgett Trough (air/water interface)
> 如果细胞膜是磷脂双分子层，那么把细胞膜摊平铺成一个单层，其面积应该是细胞表面积的两倍
...
Phospholipids will spontaneously form spherical bilayers called vesicles or liposomes
...
They flip-flop rarely (less than once per month) without the help of enzymes

- plasma mambrane: asymmeric liquid bilayer

**the fluidity of a lipid bilayer depends on its composition**

- unsaturated hydrocarbon chains with cis-double bonds(会更短)
...
The plasma membrane has an asymmetrical distribution of glycolipids and phospholipids 

The **orientation** of the membrane relative to the cytosol is **preserved** during vesicle transport
...
**Examine the general structure and properties of membrane proteins**
...
- Less common is the beta-barrel: Form wide channels - (porins)
...
- Using hydropathy plots to localize potential α-helical membrane-spanning segments in a polypeptide chain

> 疏水性图（最常见的是 Kyte-Doolittle 曲线）是一种通过计算蛋白质序列中氨基酸的疏水/亲水性质，从而预测蛋白质结构（特别是跨膜结构域）的强有力工具。
> 正峰（高于基线）：代表疏水性区域。如果峰值达到阈值（通常在 +1.6 左右）且宽度合理，通常暗示存在跨膜片段。
> 负谷（低于基线）：代表亲水性区域，通常位于蛋白质表面或暴露在胞质/胞外环境中。

- Two short α helices in the aquaporin water channel, each of which spans only halfway through the lipid bilayer
...
form from different numbers of beta-strands，不同strands数量
...
A single-pass transmembrane protein（单次跨膜蛋白）
> 信号转导 (Signal Transduction)：许多单次跨膜蛋白是受体（例如受体酪氨酸激酶，RTK）。
> - 胞外域：负责捕捉外界的信号（如生长因子）。
> - 跨膜域：负责将信号从膜外传导至膜内。
> - 胞内域：通常具有激酶活性，捕捉到信号后会发生构象改变（如二聚化），从而在胞内触发级联反应。
> 细胞黏附 (Cell Adhesion)：例如某些细胞表面标记物，只需通过单次锚定即可将细胞连接到基质或其他细胞上。

**membrane proteins can be solubilized and purified in detergents**

SDS: ionic detergents
Detergents are amphipathic and can disrupt lipid bilayers and solubilize integral membrane proteins
...
- Integral membrane proteins (transmembrane, lipid linked, monolayer associated) must be solubilized with detergent to remove them from the membrane for study
- Peripheral membrane proteins can be released by gentle methods which leave the membrane intact

**Bacteriorhodopsin**

- is a light-driven proton pump 7 aa-helix

using Retinal as light-sensor, Retinal is vitamin A in its aldehyde form and is identical to the chromophore found in rhodopsin of the photoreceptor cells of the vertebrate eye.
...
- Protein Crowding: 大量的蛋白质被招募到膜的特定区域时，它们之间会发生物理碰撞和空间排斥。为了缓解这种相互挤压带来的侧向压力，膜会向压力较小的一侧弯曲，从而增加表面积以容纳这些密集的蛋白质
...
> Urinary Bladder
> - 屏障功能：防止尿液中的代谢废物和电解质回流进组织。
> - 机械适应：在膀胱充盈拉伸时维持细胞间结构的完整性。
> 精子的三个部位也是泾渭分明

locolization code in protein-> self assemble into large aggregates
- they can be tethered by interactions with assemblies of macromolecules (B) outside 
- or (C) inside the cell; 
...
cortical cytoskeleton: 位于细胞质膜（Plasma Membrane）内侧的一层由肌动蛋白丝（Actin filaments）、频谱蛋白（Spectrin）等构成的致密网络

The spectrin(血影蛋白)-based cytoskeleton on the cytosolic side of the human red blood cell plasma membrane. anchored by Ankyrin
...
most ions have significant different concentration in cytoplasmic and extracellular
