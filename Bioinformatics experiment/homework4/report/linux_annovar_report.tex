...
\subsection{创建课程目录并验证当前位置（1 分）}

\noindent\textbf{得分点要求：} 从 home 目录开始，创建课程文件夹 \texttt{bioinfor}，进入该目录，并用 \texttt{pwd} 验证。
...
\subsection{复制 ANNOVAR 压缩包并用 ls 验证（0.5 分）}

\noindent\textbf{得分点要求：} 将 ANNOVAR 安装包复制到课程目录，并用 \texttt{ls} 查看是否复制成功。
...
\subsection{用 bash 删除复制出的压缩包并验证（0.5 分）}

\noindent\textbf{得分点要求：} 使用 \texttt{rm} 删除课程目录中的压缩包，并再次用 \texttt{ls} 验证。
...
\subsection{运行并解释 tar 解压命令（1 分）}

\noindent\textbf{得分点要求：} 运行并解释 \texttt{tar -zxvf annovar.latest.tar.gz}，写明帮助来源。
...
\noindent\textbf{得分点要求：} 使用 \texttt{cat} 查看小文本文件，理解 ANNOVAR 示例目录中的文件说明。
...
\subsection{用 less 查看 FASTA 并搜索 chr22（1 分）}

\noindent\textbf{得分点要求：} 使用 \texttt{less} 查看大 FASTA 文件，并在 \texttt{less} 中搜索 \texttt{chr22}。
...
FASTA 文件较大，不适合一次性用 \texttt{cat} 全部输出；\texttt{less} 可以分页查看并用 \texttt{/chr22} 搜索包含 chr22 的序列描述行。

\subsection{筛选 chr21 行并导出为 hg38\_chr21.txt（1 分）}
...
\subsection{Bash 脚本统计 chr21 序列数量（1 分）}
...
\subsection{在脚本中检查输入文件是否存在（1 分）}

\noindent\textbf{得分点要求：} 在上一题脚本基础上，加入输入文件存在性判断。
...
\subsection{循环比较所有常染色体 RNA 序列数量（1 分）}
...
\subsection{使用 R 可视化结果（1 分）}

\noindent\textbf{得分点要求：} 使用 R 可视化所有染色体 RNA 序列数量，并比较常染色体长度与 RNA 序列数量。
...
\subsection{讨论 RNA 序列数量是否与染色体长度成比例（1 分）}

\noindent\textbf{得分点要求：} 结合可视化结果，讨论每条染色体 RNA 序列数量是否与 DNA 长度成比例，并分析影响因素。
